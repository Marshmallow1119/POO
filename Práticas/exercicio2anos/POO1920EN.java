...
public class POO1920EN {
	...
	private static void alinea2(PrintStream out) {
		...
		if (loja != null) {
			...
			try (Scanner sc = new Scanner(new File("./src/exercicio2anos/epoo20.txt"))) {
				String[] line;
				Telemovel temp1;
				...
				while (sc.hasNextLine()) {
					...
					String[] find = lin.split(";");
					switch(find[0]){
					case"TELEM" :
						marca = find[1];
						modelo = find[2];
						preco = Integer.parseInt(find[3].replaceAll("\\s+",""));
						stock = Integer.parseInt(find[4].replaceAll("\\s+",""));
						temp1 = new Telemovel(marca, modelo, preco);
						temp1.setStock(stock);
						loja.add(temp1);
						break;
					case "ELECT" :
						String tipo = find[1];
						marca = find[2];
						modelo = find[3];
						preco = Integer.parseInt(find[4].replaceAll("\\s+",""));
						stock = Integer.parseInt(find[5].replaceAll("\\s+",""));
						ClasseEnergetica energ = ClasseEnergetica.valueOf(find[6]);
						temp2 = new Electrodomestico(tipo,marca,modelo,preco);
						temp2.setStock(stock);
						temp2.setClasse(energ);
						loja.add(temp2);
						break;
					case "LIVRO":
						String title = find[1];
						preco = Integer.parseInt(find[2].replaceAll("\\s+",""));
						stock = Integer.parseInt(find[3].replaceAll("\\s+",""));
						List<Autor> aut= new ArrayList<>();
						String[] au = find[4].split(",");
						for(int i = 0; i==au.length;i++){
							String nome = au[i];
							Autor a = new Autor(nome);
							aut.add(a);
						}
						temp3 = new Livro(title,preco, aut);
						temp3.setStock(stock);
						loja.add(temp3);
						break;
				}
				}
			...
			for(Produto p: prods){
				out.printf("%-40s%10.2f%n",  p.getDescricao(), p.precoVendaAoPublico());
			}
